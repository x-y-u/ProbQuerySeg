...
def merge_small_segments(unique_combos, order, inverse_idx, min_voxels):
    ...
    segments = [{"main_index": i, "indices": [i], "count": counts[i]} for i in order]
...
def multi_channel_to_label_cluster(arr, threshold=0.6, min_voxels=500):
    ...
    for seg in segments:
        cur_mask = unique_combos[seg['main_index']]
        for index in seg["indices"]:
            combo_to_label[tuple(unique_combos[index])] = cur_mask

    combo_to_label[tuple(np.zeros(C, dtype=np.int32))] = np.zeros(C, dtype=np.int32)

    labels = np.zeros((origin_flat.shape[0], C), dtype=np.int32)

    for i, row in tqdm(enumerate(origin_flat)):
        labels[i] = combo_to_label[tuple(row)]

    return labels.reshape(D, H, W, C), combo_to_label


def main():

    in_path = "/media/UG3/xieyu/tractography_generate/HCP/HCP_multi_mask_reg_plus/multi_mask_plus_filtered_norm.nii.gz"
    out_path = "/media/UG3/xieyu/tractography_generate/HCP/HCP_multi_mask_reg_plus/multi_mask_remap.nii.gz"

    origin_image_nii = nib.load(in_path)

    origin_image = origin_image_nii.get_fdata()

    new_image, unique_combos = multi_channel_to_label_cluster(origin_image, threshold=0.6, min_voxels=500)
